...
get_anim_description() {
  if [[ "$CHARACTER" =~ [Ww]orm ]]; then
    case "$1" in
      IDLE)
        echo "IDLE slithering cycle: subtle gentle breathing expansion, body resting on ground with slight rhythmic pulsing, segmented body coiled slightly, head turning micro tilts, six subtle idle worm poses with character nearly stationary"
        ;;
      WALK)
        echo "WALK cycle: slithering forward motion, body segments compressing and extending in a wave pattern, tail sliding along ground, six slithering poses showing horizontal locomotion"
        ;;
      RUN)
        echo "RUN cycle: fast slithering motion, quick undulating wave-like contractions across body segments, head reared slightly forward, six fast slithering poses with high speed and energy"
        ;;
      ATTACK)
        echo "ATTACK cycle: rearing head and front body segments back to wind up, then lunging forward with gaping mouth, then retracting back, six biting lunge attack poses from wind-up to bite impact"
        ;;
      JUMP)
        echo "JUMP cycle: coiling body on ground, then launching forward-upward into the air, body stretching straight at peak of arc, then coiling back for landing, six jumping poses through vertical arc"
        ;;
      HURT)
        echo "HURT cycle: knocked backward, body recoils in pain, segments twisting and bending back, glowing purple eyes showing distress, six hurt reaction poses"
        ;;
      DEATH)
        echo "DEATH cycle: body collapsing sideways, segment joints loosening, lying flat on side, turning pale and dissolving into spectral residue, six death poses from collapse to static dead body"
        ;;
      SPECIAL)
        echo "SPECIAL cast cycle: rearing up tall, glowing purple magical energy radiating around body segments, mouth open emitting a purple sonic scream, six special magical casting poses with aura"
        ;;
      *)
...
OPTIONS
  ...
  --skip-generate          Skip the mmx generation stage and use existing raw JPEGs.
  --row-width <px>         Each row's pixel width. Default: 1280
  --key-color-hex <hex>    Hex color code for chroma key. Default: #FF00FF
  --key-color-name <name>  Color name for chroma key. Default: magenta
  --fuzz <percent>         ImageMagick chroma-key fuzz percentage. Default: 35
  -h | --help              This message.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --row-width) ROW_WIDTH="$2"; shift 2 ;;
    --key-color-hex) KEY_COLOR_HEX="$2"; shift 2 ;;
    --key-color-name) KEY_COLOR_NAME="$2"; shift 2 ;;
    --fuzz) FUZZ="$2"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
MAGENTA="$KEY_COLOR_HEX"
SHEET_WIDTH=$((ROW_WIDTH + LABEL_WIDTH))
PROMPT_PREFIX="Pure flat 2D anime cel shaded horizontal sprite strip. ${FRAMES} sequential frames left to right of the SAME character: ${CHARACTER}. Identical scale and design across all ${FRAMES} frames, all facing right side profile. Pure solid ${KEY_COLOR_NAME} ${MAGENTA} background edge to edge filling all empty space, no scenery, no ground, no floor, no shadows on background, no text, no labels, no frame boxes. ${FRAMES} poses evenly spaced left to right with small ${KEY_COLOR_NAME} padding between frames. "
PROMPT_SUFFIX="."
...
if [[ "$SKIP_GENERATE" -eq 0 ]]; then
  ...
  echo "==> Skipping image generation, using existing files in $OUTPUT/raw"
fi
...
for ACTION in "${ACTIONS[@]}"; do
  ...
  magick "$OUTPUT/raw/_base_${ACTION}.png" \
    -font DejaVu-Sans-Bold -pointsize 44 -fill black \
